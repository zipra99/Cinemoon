...
export class TicketInfoService {
    ...
    getListFoodFromFirebase() {
        ...
        const foodDoc = this.afs.doc<any>('foods/4820d1zy7KK4XL4OGcqp').valueChanges();
        if (foodDoc) {
            foodDoc.subscribe(data => {
                for (const item of data.listFood) {
    ...
    getListSoldSeat() {
        ...
        const snapshot = this.afs.collection('seatChoice').snapshotChanges();
    ...
    getListSoldSeatName() {
        ...
        const dayStr = `${this.day.getDate()}/${this.day.getMonth() + 1}`;
        for (const data of this.listSeatSold) {
    ...
    updateListSoldSeat() {
        ...
        const dayStr = `${this.day.getDate()}/${this.day.getMonth() + 1}`;
        for (const item of this.listSeatSold) {
            if (item.day === dayStr && item.time === this.time && item.name === this.movieDetail.asciiName) {
                data = item;
            }
        }
        if (data) {
            const userDoc = this.afs.doc<any>(`seatChoice/${data.id}`);
            userDoc.update({
                listSoldSeat: this.listSeatSoldName
            });
        } else {
            const docId = this.afs.createId();
            const userDoc = this.afs.doc<any>(`seatChoice/${docId}`);
            userDoc.set({
                id: docId,
                time: this.time,
    ...
    randomQRCodeUrl(length) {
        const output = 'https://api.qrserver.com/v1/create-qr-code/?size=512x512&data=';
        let randomStr = '';
